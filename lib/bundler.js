...
var Bundler = module.exports = function(config) {
    ...
    this.config = deap.updateShallow({
        ...
        watchify: false, // if watchify should be used - bundle source is cached when it is
        transforms: null, // an array of transforms
        debug: false, // browserify debug flag - generates sourcemaps if true
        // by default, modules are not "requireable" at the top level
        require: function(module) {
            return false;
        },
        // can adjust alias module is exposed as
...
Bundler.prototype = deap({
    ...
    createBundle: function(module, done) {
        ...
        if(this.config.require(module)) {
            bundle.require(modulePath, { expose: this.config.expose(module) });
        }else {
